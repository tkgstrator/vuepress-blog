...
### [バックアップのインストール](/posts/2021/10/02/goldleaf.html)

バックアップのインストールは Goldleaf を使う方法が一番便利だと思います。

::: tip Goldleaf について

Goldleaf は圧縮形式である`NSZ`や`XCZ`に対応していない。

が、当ブログでは圧縮形式は不要だと考えています。何故なら、現在大容量 HDD は非常に安価であり数%の容量削減のために時間をかけて圧縮することがコストに見合っていないこと。更に`NSZ`や`XCZ`はインストール時に`NSP`や`XCI`に比べて復号の手間がかかるため時間がかかることなどが挙げられます。
